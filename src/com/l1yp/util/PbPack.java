...
public class PbPack {
    ...
    private void UpdateFathersNode(ArrayList<String> list, int valType, int keyLen, int valLen, int lenLen) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (thisNode == null) throw new RuntimeException("找不到当前路径的节点[" + thisPath + "]");
    ...
    private void InitFathersNode(ArrayList<String> list) {
        for (int i = list.size() - 1; i >= 0; i--) {
            ...
            if (map.containsKey(thisPath)) {
                ...
                if (fatherNode == null) throw new RuntimeException("找不到[" + thisPath + "]的父节点");
                Node<?> n = new Node<>();
                ...
                n.key = GetKeyForPath(thisPath, WireFormatMicro.WIRETYPE_LENGTH_DELIMITED);
                n.lenLen = 0;
